...
\begin{algorithm}
	\KwIn{TskList, une ThreadsTaskList}
	\KwOut{Un booléen indiquant s'il reste encore des tâches à calculer ou non\\\hfill\\}
	
	\If(\tcp*[h]{Il reste encore des tâches à donner}){TskList.totalTaskGiven < TskList.totalTaskCount}
	{
		TskList.incrementTotalTaskGiven()\\
		calculerTile(TskList.totalTaskGiven - 1)\tcp*[l]{On calcule la tâche numéro totalTaskGiven - 1}
		TskList.incrementTotalTaskFinished()\\\hfill\\

		\Return{TRUE}
	}
	\Else(\tcp*[h]{Il n'y a plus de tâches à donner, ce thread peut arrêter les calculs})
	{
		\Return{FALSE}
	}
