...
import raytracer.accelerationStructures.AccelerationStructure;
import raytracer.geometry.Shape;
import raytracer.geometry.shapes.Plane;
import raytracer.geometry.shapes.Sphere;
import raytracer.materials.Material;
import raytracer.maths.ColorOperations;
import raytracer.maths.MatrixD;
import raytracer.maths.Point;
import raytracer.maths.Ray;
import raytracer.maths.Vector;
import raytracer.multithreading.ThreadsTaskList;
import raytracer.multithreading.TileTask;
import raytracer.multithreading.TileThread;
...
public class RayTracer
{
	...
	public RayTracer(int renderWidth, int renderHeight)
	{
		...
		this.rtStats.setTotalPixelCount(renderHeight * renderWidth);
	...
	protected Color computeReflectionsColor(RayTracingScene renderScene, RayTracerInterInfos intInfos, int depth)
	{
		...
		if(intObjMat.getReflectiveCoeff() > 0)
		{
			...
			int blurSampleCount = (this.settings.isEnableBlurryReflections() && intInfos.getIntObjMat().getRoughness() > 0 && intInfos.getIntObjMat().getRoughness() < 1) ? this.settings.getBlurryReflectionsSampleCount() : 1;
	...
	protected void computePartialImage(RayTracingScene renderScene, int startX, int startY, int endX, int endY)
	{
		...
		for(int y = startY; y < endY; y++)
		{
			for(int x = startX; x < endX; x++)
			{
				...
				this.rtStats.incrementNbPixelsComputed();
			}
